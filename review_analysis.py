...
EXCEL_FILE_PATH = r"C:\Users\Admin\Desktop\Review analysis\amazon_reviews.xlsx"  # Updated to new path
...
REVIEW_CATEGORIES = {
    ...
    'functionality': ["doesn't work", 'Stopped working within 2 months', 'not working', 'failed', 'issues', 'error']
}

def analyze_reviews(excel_file):
    try:
        print(f"Reading Excel file: {excel_file}")
        df = pd.read_excel(excel_file)  # Updated to read from Excel file

        if RATING_COLUMN_NAME not in df.columns:
            raise ValueError(f"Rating column '{RATING_COLUMN_NAME}' not found in Excel file")
        if COMMENT_COLUMN_NAME not in df.columns:
            raise ValueError(f"Comment column '{COMMENT_COLUMN_NAME}' not found in Excel file")
        ...
        # Save the Excel file with chart
        output_file = "ratings_distribution_with_chart.xlsx"
        wb.save(output_file)
...
def categorize_negative_reviews(negative_reviews):
    try:
        ...
        # Use the working MongoClient connection string with timeout
        client = MongoClient(
...
def main():
    try:
        ...
        negative_reviews = analyze_reviews(EXCEL_FILE_PATH)  # Updated to Excel file path
